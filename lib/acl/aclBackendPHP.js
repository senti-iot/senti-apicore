...
const TestResources = require('./dataClasses/TestResources')
...
class aclBackendPHP {
	...
	async testResources(entityUUID, resourceUUIDs, privileges) {
		return await this.apiCall.post('test/' + entityUUID + '/' + privileges.join('|'), resourceUUIDs).then(rs => {
			return new TestResources(rs.data)
		})
	}
