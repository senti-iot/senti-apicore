...
const { ENCRYPTION_KEY, REACT_APP_ENCRYPTION_KEY } = keys // Must be 256 bytes (32 characters)
...
const decryptReact = (text) => {
	let textParts = text.split(':')
	let iv = new Buffer.from(textParts.shift(), 'hex')
	let encryptedText = new Buffer.from(textParts.join(':'), 'hex')
	let decipher = crypto.createDecipheriv('aes-256-cbc', new Buffer.from(REACT_APP_ENCRYPTION_KEY), iv)
	let decrypted = decipher.update(encryptedText)
...
module.exports = { decrypt, encrypt, encryptReact, decryptReact }